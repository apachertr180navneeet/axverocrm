  ...
  public function index()
{
    ...
    if (request()->ajax() && request()->has('term')) {
        ...
        $query = UserChat::with(['fromUser' => function ($q) {
            $q->withCount(['unreadMessages']);
        }, 'toUser' => function ($q) {
            $q->withCount(['unreadMessages']);
        }])->whereIn('id', $messageIds)->orderByDesc('id');
   ...
   public function store(ChatStoreRequest $request)
{
    ...
    if($request->types == 'chat')
    {
        ...
        $query = UserChat::with('fromUser', 'toUser')
  ...
  public function fetchUserMessages($receiverID)
{
    ...
    if (!in_array('admin', user_roles())) {
        $chatExists = UserChat::where(function ($q) use ($receiverID) {
            $q->where('from', user()->id)->where('to', $receiverID);
        })->orWhere(function ($q) use ($receiverID) {
            $q->where('from', $receiverID)->where('to', user()->id);
        })->exists();
