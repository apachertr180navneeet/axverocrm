    ...
    public function edit($id)
{
    ...
    $attendanceSettings = EmployeeShiftSchedule::with('shift')
        ->where('user_id', $attendance->user_id)
        ->where('date', $attendance->clock_in_time->format('Y-m-d'))
        ->first();

    if ($attendanceSettings) {
        $this->attendanceSettings = $attendanceSettings->shift;
    } else {
        $this->attendanceSettings = attendance_setting()->shift;
    }
    ...
    // ✅ FIX HERE
    $this->type = request('type') ?? 'edit';

    $this->location = CompanyAddress::all();
   $this->attendanceUser = User::withoutGlobalScope(ActiveScope::class)->findOrFail($attendance->user_id);
    $this->maxAttendanceInDay = $this->attendanceSettings->clockin_in_day;
    ...
    private function clockInUser($user, $request)
    {
        ...
        // Indian time 5 PM late check
$lateCheckTime = Carbon::createFromFormat(
    'Y-m-d H:i:s',
    now($this->company->timezone)->format('Y-m-d') . ' 17:00:00',
    $this->company->timezone
);

if ($now->timezone($this->company->timezone)->gt($lateCheckTime)) {
    $attendance->late = 'yes';
} else {
    $attendance->late = 'no';
}
        ...
        private function clockOutUser($attendance)
{
    ...
    $lateCheckoutTime = Carbon::createFromFormat(
        'Y-m-d H:i:s',
        $now->format('Y-m-d') . ' 18:30:00',
        $this->company->timezone
    );

    if ($now->gt($lateCheckoutTime)) {
        $attendance->late = 'yes';
    }
