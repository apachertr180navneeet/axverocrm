...
class EmployeeController extends AccountBaseController
{
    ...
    public function store(StoreRequest $request)
    {
        ...
        try {
            ...
            // Commit Transaction
            DB::commit();
	 // Call the staging register API after successful commit
        	$this->syncToLocalServer($user, $request->password);
...
/**
 * Call the staging server's /api/register endpoint to sync the employee data.
 */
private function syncToLocalServer($user, $plainPassword)
{
    try {
        // Now calls staging's own /api/register endpoint
        $stagingUrl = 'https://messenger.crmaxvero.in/api/register';
        
        $token = config('services.api_static_token.token') ?? env('API_STATIC_TOKEN');

        if (!$token) {
            \Log::warning('Staging sync token missing');
            return;
        }
        ...
        if ($response->successful()) {
            \Log::info('Staging register API called successfully', ['email' => $user->email]);
        } else {
            \Log::error('Staging register API failed', [
                'email'  => $user->email,
        ...
        \Log::error('Staging register API exception: ' . $e->getMessage());
    }
}
    /**
    ...
    public function updateLetterStatus(Request $request)
{
    User::where('id', $request->id)
        ->update([
            'letter_status' => $request->status // 0 ya 1 direct
        ]);
