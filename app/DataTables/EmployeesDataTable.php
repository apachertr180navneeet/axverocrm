...
class EmployeesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            return $action;
                ...
                $datatables->addColumn('letter_status', function ($row) {
            $checked = isset($row->letter_status) && $row->letter_status ? 'checked' : '';
        
            return '<label class="switch">
                        <input type="checkbox" class="change-letter-status" data-id="'.$row->id.'" '.$checked.'>
                        <span class="slider round"></span>
                    </label>';
        });
        $datatables->addColumn('is_premium', function ($row) {
        $checked = isset($row->is_premium) && $row->is_premium ? 'checked' : '';
    
        return '<label class="switch">
                    <input type="checkbox" class="change-premium-status" data-id="'.$row->id.'" '.$checked.'>
                    <span class="slider round"></span>
                </label>';
    });
        ...
        $datatables->editColumn('name', function ($row) {
            ...
            $view = view('components.employee', ['user' => $row])->render();
              if ($row->is_premium) {
                $view .= ' <span style="color: gold; font-size: 16px;" title="Premium">★</span>';
            }
       ...
       $datatables->addColumn('designation_name', fn($row) => $row->employeeDetail?->designation?->name ?? '--');
        ...
        $datatables->rawColumns(array_merge(['name', 'action', 'role', 'status', 'check', 'employee_id','letter_status', 'is_premium'], $customFieldColumns));
    ...
    // Check if the inactive_date is in the past
        ...
        })->whereNotNull('employee_details.user_id');
        }
        }
