...
<div class="mt-4">
    ...
    <div class="p-3 bg-white rounded border">
        <div id="edit-attendance-container"></div>
    </div>
</div>
<script>
    ...
    function addAttendance(userID) {
        ...
        let day = attendanceDate[2].split(' ')[0];
  ...
  function loadFormInContainer(url) {
    $.ajax({
        url: url,
        success: function(response) {
            var tempDiv = $('<div>').html(response);
            
            // Pehle modal-body try karo
            var body = tempDiv.find('.modal-body');
            var footer = tempDiv.find('.modal-footer');
            ...
            if (body.length) {
                ...
                // modal-body nahi mila toh poori response lo
                content = response;
            }
            ...
            if (!content || content.trim() === '') {
                $('#edit-attendance-container').html('<p class="text-muted p-3">Form load nahi hua.</p>');
                return;
            }

            $('#edit-attendance-container').html(content);

            $('#clock-in-time').timepicker({
                <?php if(company()->time_format == 'H:i'): ?> showMeridian: false, <?php endif; ?>
                minuteStep: 1
            });
        ...
        error: function(xhr) {
            ...
            $('#edit-attendance-container').html('<p class="text-danger p-3">Error: ' + xhr.status + '</p>');
        }
    });
}
  ...
  setTimeout(function () {
    var attendanceId = "<?php echo e($attendance->id); ?>";

    // Pehle check karo ID exist karti hai
    var editUrl = "<?php echo e(route('attendances.edit', ':id')); ?>?type=edit";
    editUrl = editUrl.replace(':id', attendanceId);
    ...
    $.ajax({
        ...
        error: function(xhr) {
            // 404 ya koi bhi error = add form load karo
            console.log('Edit failed (' + xhr.status + ') for id: ' + attendanceId + ' → loading add form');
            loadAddForm();
        }
    });
}, 200);

    // Activity list se edit button click
   function editAttendance(id) {
    var url = "<?php echo e(route('attendances.edit', ':id')); ?>?type=edit";
    url = url.replace(':id', id);
    ...
    $.ajax({
        ...
        success: function(response) {
            var tempDiv = $('<div>').html(response);
            var body = tempDiv.find('.modal-body');
            var content = body.length ? body.html() : response;

            if (tempDiv.find('.modal-footer').length) {
                content += tempDiv.find('.modal-footer').prop('outerHTML');
            }

            if (!content || content.trim() === '') {
                console.log('Empty for id: ' + id);
                return;
            }
        ...
        error: function(xhr) {
            console.log('Edit 404 for id: ' + id);
            // Specific id ke liye kuch nahi karo
        }
    });
}
...
<style>
#edit-attendance-container .modal-footer {
    display: flex;
    justify-content: flex-end;
    margin-top: 15px;
}

#edit-attendance-container .modal-body {
    padding: 0;
}
</style>
<?php /**PATH /home/u972418887/domains/kactto.com/public_html/crmhr/resources/views/attendances/ajax/show.blade.php ENDPATH**/ ?>